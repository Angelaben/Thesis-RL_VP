...
class Logger:
   ...
   #     print("Best buy : ", best_buy)
    ...
    def plot(self):
        ...
        plt.axis([0, min(len(self.log_mean_reward), self.lim), 0, 1])
       ...
       # plt.axis([0, len(self.log_per_client_reward_mean), 0, 2])
        ...
        plt.plot(self.log_mean_opti[:min(len(self.log_per_client_reward_mean[index_client]), self.lim)], label = "Mean ref optimal")
        plt.grid()
        plt.legend()
        plt.show()
